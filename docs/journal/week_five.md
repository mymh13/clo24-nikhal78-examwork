...
**Hot-Reload Issue & Fix:** Initially, hot-reload was not working - feature flag changes required service restart to take effect. Root cause: Missing `app.UseAzureAppConfiguration()` middleware in the HTTP pipeline. This middleware is required to trigger configuration refresh on each HTTP request. Added middleware to `WebApplicationExtensions.cs` after static files but before routing. Hot-reload now works correctly - updating sentinel key triggers automatic refresh within 1 minute without restart. Documented in ADR-014 with correct implementation details.
